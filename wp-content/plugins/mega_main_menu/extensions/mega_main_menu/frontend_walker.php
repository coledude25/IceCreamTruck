 ...
 */
	class Mega_Main_Menu_Frontend_Walker extends Walker_Nav_Menu {
		...
		function default_menu_item( &$output, $args, $item, $depth ) {
			...
			if ( $depth == '1' && get_post_meta( $args->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_type', true) == 'multicolumn_dropdown' ) {
...
//            $item_output .= $args->before;
			$item_output .= mm_common::ntab( $depth + 1 ) . '<' . ( $_disable_link !== true ? 'a' : 'span' ) . $attributes .' tabindex="0">';
...
//            $item_output .= $link_before;
			...
			$item_output .= mm_common::ntab( $depth + 3 ) . apply_filters( 'the_title', $item->title, $item->ID );
			if ( !empty( $item->descr ) ) {
...
*/
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			global $mega_main_menu;
			$args = (object)$args;
			$img = ( (string)$depth == '0' && get_post_meta( $args->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_bg_image', true) ) 
				? get_post_meta( $args->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_bg_image', true) 
				: 'no-img';
		...
		function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$args = (object)$args;
			$indent = str_repeat( "\t", $depth );
				$mmm_submenu_type = ( get_post_meta( $args->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_type', true) ) ? get_post_meta( $args->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_type', true) : 'default_dropdown';
...
*/
			...
			$args[0]->_item_style = ( get_post_meta( $element->ID, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_item_style', true ) != false ) 
				...
				: '';
			$mmm_submenu_type = ( get_post_meta( $args[0]->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_type', true) ) 
				? get_post_meta( $args[0]->menu_main_parent, $mega_main_menu->constant['MM_WARE_PREFIX'] . '_submenu_type', true) 
				: 'default_dropdown';
