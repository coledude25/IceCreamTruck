	...
	function mm_options_generator( $option, $mm_saved_value = false, $current_class = 'none' ){
		...
		switch ( $option['type'] ) {
			...
			case 'icons':
				...
				$out .= mm_common::ntab(11) . '<span class="input-group-btn">';
				$out .= mm_common::ntab(12) . '<a data-toggle="modal" href="' . admin_url() . '?mm_page=icons_list&input_name=' . esc_attr( $option['key'] ) . '&modal_id=icons_list_' . esc_attr( $clear_full_key ) . '" data-target="#icons_list_' . $clear_full_key . '" class="btn btn-primary">' . __( 'Show Icons', $current_class->constant[ 'MM_TEXTDOMAIN_ADMIN' ] ) . '</a>';
				$out .= mm_common::ntab(11) . '</span><!-- class="input-group-btn" -->';
				$out .= mm_common::ntab(10) . '</div><!-- class="input-group input-group-sm col-xs-9" -->';
				...
				$out .= mm_common::ntab(10) . '</div><!-- class="col-xs-3" -->';
				$out .= mm_common::ntab(9) . '</div><!-- class="row" -->';
				$out .= mm_common::ntab(9) . '<div id="icons_list_' . $clear_full_key . '" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="icons_listLabel" aria-hidden="true">';
				$out .= mm_common::ntab(10) . '<div class="bootstrap">';
				$out .= mm_common::ntab(11) . '<div class="modal-dialog">';
				$out .= mm_common::ntab(12) . '<div class="modal-content">';
				$out .= mm_common::ntab(12) . '</div><!-- class="modal-content" -->';
				$out .= mm_common::ntab(11) . '</div><!-- class="modal-dialog" -->';
				$out .= mm_common::ntab(10) . '</div><!-- class="bootstrap" -->';
				$out .= mm_common::ntab(9) . '</div><!-- class="modal" -->';
				break;
			case 'caption':
...
*/
			...
			case 'color':
				...
				$out .= mm_common::ntab(9) . '<div class="mm_must_be_colorpicker input-append color input-group input-group-sm col-xs-3" data-color="' . $value . '" data-color-format="rgba" id="' . $clear_full_key . '_colorpicker">';
			...
			case 'font':
				...
				if ( $option['values'] == '' || ( is_array( $option['values'] ) && in_array( 'font_size', $option['values'] ) ) ) {
					...
					$out .= mm_common::ntab(9) . '<input class="form-control col-xs-12" type="number" step="1" min="4" max="120" name="' . $option['key'] . '[font_size]" value="' . ( ( isset( $mm_saved_value['font_size'] ) && $mm_saved_value['font_size'] !== false ) 
		...
		if ( $option['type'] != 'collapse_start' && $option['type'] != 'collapse_end' && $option['type'] != 'skin_options_generator' && $option['type'] != 'caption' ) {
			...
			$section .= mm_common::ntab(7) . '<div class="option row ' . str_replace( array( $current_class->constant[ 'MM_OPTIONS_NAME' ], '[',']'), '', $option['key'] ) . ' ' .  $option['type'] . '_type"' . ( ( isset( $option['dependency']['element'] ) && isset( $option['dependency']['value'] ) ) ? ' data-dependencyelement="' . $option['dependency']['element'] . '" data-dependencyvalue="' . implode( '|', $option['dependency']['value'] ) . '"' : '' ) . '>';
